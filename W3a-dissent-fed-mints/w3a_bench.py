...
def raw_call(system, user):
    """One HTTP attempt in the current protocol mode -> (content, secs).
    Applies registered fallback on think-key rejection."""
    modes = []
    if PROTO["mode"] == "thinkfalse":
        modes = [(True, system)]
    elif PROTO["mode"] == "nothink":
        modes = [(False, system + " /no_think")]
    else:
        modes = [(True, system), (False, system + " /no_think")]
    last_err = None
    for use_think_key, sysmsg in modes:
        ...
        try:
            ...
            last_err = f"HTTP {e.code}: {e.read()[:120]}"
            if use_think_key:
                continue          # registered fallback: retry without key
            raise
    raise RuntimeError(f"no protocol succeeded: {last_err}")
...
def query_item(prompt):
    ...
    for attempt in (1, 2, 3):
        ...
        raw = content
        m = re.search(r"\b(yes|no)\b", content, re.I)
        if m:
            return m.group(1).lower(), dt, attempt, content[:80]
    return "UNPARSED", dt, 3, raw[:80]

def resolve_protocol():
    """Warm-up decides thinkfalse vs nothink (registered tree)."""
    probe = ("Conditions: dew point depression 1.5°C, wind 5 kt. "
             "Question: is fog likely within the next few hours?")
    # try think:false first
    payload = dict(model=MODEL, stream=False, keep_alive="30m",
    ...
    try:
        with urllib.request.urlopen(req, timeout=180) as r:
            body = json.loads(r.read())
        if "<think>" not in body["message"]["content"]:
            PROTO["mode"] = "thinkfalse"
            return f"think:false ok -> {body['message']['content'][:60]!r}"
        raise ValueError("thinking leaked despite think:false")
    except Exception as e:
        payload["messages"][0]["content"] = SYSTEM + " /no_think"
        payload.pop("think")
        req = urllib.request.Request(
            URL, data=json.dumps(payload).encode(),
            headers={"Content-Type": "application/json"})
        with urllib.request.urlopen(req, timeout=180) as r:
            body = json.loads(r.read())
        PROTO["mode"] = "nothink"
        return (f"fallback /no_think (primary failed: {type(e).__name__}: "
                f"{str(e)[:80]}) -> {body['message']['content'][:60]!r}")
...
def stage_results(corpus):
    t1 = load("w3a-ledger-train1.json"); s1 = load("w3a-ledger-test1.json")
    m1 = load("w3a-mint1.json")
    t2 = load("w3a-ledger-train2.json"); s2 = load("w3a-ledger-test2.json")
    m2 = load("w3a-mint2.json"); dis = load("w3a-dissent.json")
    ...
    results = dict(
        meta=dict(experiment="W3a — the upgraded judge: dissent-fed mints",
                  station=2, model=MODEL, temp=0.3, seed=20260827,
                  protocol=t1["protocol"],
                  run_at=time.strftime("%Y-%m-%d %H:%M:%S %Z"),
                  registered="REGISTRATION.md (2026-08-26 23:17 AKDT)",
                  replaces="W3 as designed (W2 verdict recommendation)"),
...
def main():
    corpus = load("w3a-corpus.json")
    if corpus is None:
        sys.exit("no corpus — run corpus_gen.py first")
    if PROTO["mode"] is None:
        print("warm-up:", resolve_protocol(), flush=True)
    # Stage 0: G0 guard train ledger
    t1 = load("w3a-ledger-train1.json")
    if t1 is None:
        t1 = run_ledger(corpus, "train", None, "w3a-ledger-train1.json", "train1")
    tp = [r for r in t1["records"] if r["ans"] != "UNPARSED"]
    ...
    if g0 < G0_BAR:
        ...
        return
    # Round 1
    if load("w3a-mint1.json") is None:
        mint_from("w3a-ledger-train1.json", "w3a-mint1.json")
    if load("w3a-ledger-test1.json") is None:
        run_ledger(corpus, "test", None, "w3a-ledger-test1.json", "test1")
    # Dissent set
    dis = load("w3a-dissent.json")
    if dis is None:
        m1 = load("w3a-mint1.json")
        tab1 = np.array(m1["table"], dtype=np.int8)
        rows = []
        ...
        with open("w3a-dissent.json", "w") as f:
            json.dump(dis, f, indent=1)
        print(f"dissent set: {n_full} found, {len(rows)} fed "
              f"(YES {ys} / NO {len(rows)-ys})", flush=True)
    # Round 2
    prefix = dis["prefix"]
    if load("w3a-ledger-train2.json") is None:
        run_ledger(corpus, "train", prefix, "w3a-ledger-train2.json", "train2")
    if load("w3a-mint2.json") is None:
        mint_from("w3a-ledger-train2.json", "w3a-mint2.json")
    if load("w3a-ledger-test2.json") is None:
        run_ledger(corpus, "test", prefix, "w3a-ledger-test2.json", "test2")
